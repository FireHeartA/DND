...
type EncounterDifficulty = 'Trivial' | 'Easy' | 'Medium' | 'Hard' | 'Deadly'
...
const getEncounterMultiplier = (monsterCount: number, playerCount: number): number => {
  let multiplier = 1
  ...
  if (playerCount > 0 && playerCount < 3) {
    if (monsterCount === 1) return 1.5
    if (monsterCount === 2) return 2
    if (monsterCount >= 3 && monsterCount <= 6) return 2.5
    if (monsterCount >= 7 && monsterCount <= 10) return 3
    if (monsterCount >= 11 && monsterCount <= 14) return 4
    return 5
  }

  if (playerCount >= 6) {
    if (monsterCount === 1) return 0.5
    if (monsterCount === 2) return 1
    if (monsterCount >= 3 && monsterCount <= 6) return 1.5
    if (monsterCount >= 7 && monsterCount <= 10) return 2
    if (monsterCount >= 11 && monsterCount <= 14) return 2.5
    return 3
  }
...
export const InitiativeView: React.FC<InitiativeViewProps> = ({ onNavigateToCampaigns, resetKey }) => {
  ...
  const encounterAssessment = useMemo(() => {
    if (!activeCampaign) {
      return null
    }

    const playerEntries = orderedCombatants.filter((combatant) => combatant.type === 'player')
    const monsterEntries = orderedCombatants.filter((combatant) => combatant.type === 'monster')
    const monsterCount = monsterEntries.length

    const playerLevels = playerEntries
      .map((combatant) => {
        const sourceTemplateId = combatant.sourceTemplateId
        if (!sourceTemplateId) {
          return null
        }
        const rosterEntry = activeCampaign.playerCharacters.find((entry) => entry.id === sourceTemplateId)
        return rosterEntry?.playerLevel ?? null
      })
      .filter((level): level is number => typeof level === 'number' && Number.isFinite(level) && level > 0)

    const totalMonsterXp = monsterEntries.reduce((sum, combatant) => {
      const sourceMonsterId = combatant.sourceMonsterId
      if (!sourceMonsterId) {
        return sum
      }
      const monster = monsterLibrary.monsters[sourceMonsterId]
      if (!monster) {
        return sum
      }
      const cleanedXp = monster.challengeXp.replace(/,/g, '').trim()
      const xp = Number.parseInt(cleanedXp, 10)
      return Number.isFinite(xp) ? sum + xp : sum
    }, 0)

    const thresholds = playerLevels.reduce(
      (acc, level) => {
        const levelThreshold = XP_THRESHOLDS_BY_LEVEL[Math.min(20, Math.max(1, Math.trunc(level)))]
        if (!levelThreshold) {
          return acc
        }
        acc.easy += levelThreshold.easy
        acc.medium += levelThreshold.medium
        acc.hard += levelThreshold.hard
        acc.deadly += levelThreshold.deadly
        return acc
      },
      { easy: 0, medium: 0, hard: 0, deadly: 0 },
    )

    const multiplier = getEncounterMultiplier(monsterCount, playerLevels.length)
    const adjustedMonsterXp = Math.round(totalMonsterXp * multiplier)

    let difficulty: EncounterDifficulty = 'Trivial'
    if (adjustedMonsterXp >= thresholds.deadly && thresholds.deadly > 0) difficulty = 'Deadly'
    else if (adjustedMonsterXp >= thresholds.hard && thresholds.hard > 0) difficulty = 'Hard'
    else if (adjustedMonsterXp >= thresholds.medium && thresholds.medium > 0) difficulty = 'Medium'
    else if (adjustedMonsterXp >= thresholds.easy && thresholds.easy > 0) difficulty = 'Easy'

    return { difficulty, thresholds, adjustedMonsterXp, totalMonsterXp, multiplier, playerCount: playerLevels.length }
  }, [activeCampaign, monsterLibrary.monsters, orderedCombatants])
    ...
    <>
      <section className="main__header">
        ...
        <div className="header-actions">
          ...
          {encounterAssessment && (
            <div className="initiative-summary initiative-summary--difficulty">
              <span className="summary__label">Encounter</span>
              <span className="summary__value summary__value--compact">{encounterAssessment.difficulty}</span>
              <span className="summary__subtle">
                Adj XP {encounterAssessment.adjustedMonsterXp.toLocaleString()} (base{' '}
                {encounterAssessment.totalMonsterXp.toLocaleString()} × {encounterAssessment.multiplier})
              </span>
              <span className="summary__subtle">
                Easy {encounterAssessment.thresholds.easy.toLocaleString()} • Medium{' '}
                {encounterAssessment.thresholds.medium.toLocaleString()} • Hard{' '}
                {encounterAssessment.thresholds.hard.toLocaleString()} • Deadly{' '}
                {encounterAssessment.thresholds.deadly.toLocaleString()}
              </span>
              {encounterAssessment.playerCount === 0 && (
                <span className="summary__subtle">Add players from the campaign roster with levels to evaluate difficulty.</span>
              )}
            </div>
          )}
