...
const isLikelyTypeLine = (value) => {
  ...
  return sizeKeywords.some((keyword) => lower.includes(keyword))
}

const addUniqueTag = (tags, seen, value, options = {}) => {
  if (typeof value !== 'string') {
    return
  }
...
const normalizeTagObject = (tag) => {
  if (!tag || typeof tag !== 'object') {
    return null
  }

  const label = stripFormatting(tag.label || '')
  if (!label) {
    return null
  }
  ...
  return {
    label,
    key: typeof tag.key === 'string' && tag.key.trim() ? tag.key.trim() : undefined,
    removable: tag.removable !== false,
  }
}

export const buildMonsterTags = (monster, existingTags = []) => {
  const tags = []
  const seen = new Set()

  if (monster) {
    if (monster.hitPoints !== null && monster.hitPoints !== undefined) {
      const hpLabel = `HP ${monster.hitPoints}${
        monster.hitDice ? ` (${monster.hitDice})` : ''
      }`
      addUniqueTag(tags, seen, hpLabel, { key: 'hp', removable: false })
    }

    if (monster.armorClass !== null && monster.armorClass !== undefined) {
      const armorLabel = `AC ${monster.armorClass}${
        monster.armorNotes ? ` (${monster.armorNotes})` : ''
      }`
      addUniqueTag(tags, seen, armorLabel, { key: 'ac', removable: false })
    }

    if (monster.challengeRating) {
      const challengeLabel = monster.challengeXp
        ? `CR ${monster.challengeRating} (${monster.challengeXp})`
        : `CR ${monster.challengeRating}`
      addUniqueTag(tags, seen, challengeLabel)
    }

    if (monster.proficiencyBonus) {
      addUniqueTag(tags, seen, `PB ${monster.proficiencyBonus}`)
    }

    if (monster.typeLine) {
      addUniqueTag(tags, seen, monster.typeLine)
    }

    if (monster.speed) {
      addUniqueTag(tags, seen, `Speed ${monster.speed}`)
    }

    if (monster.savingThrows) {
      addUniqueTag(tags, seen, `Saves ${monster.savingThrows}`)
    }

    if (monster.skills) {
      addUniqueTag(tags, seen, `Skills ${monster.skills}`)
    }

    if (monster.damageVulnerabilities) {
      addUniqueTag(tags, seen, `Vulnerable ${monster.damageVulnerabilities}`)
    }

    if (monster.damageResistances) {
      addUniqueTag(tags, seen, `Resist ${monster.damageResistances}`)
    }

    if (monster.damageImmunities) {
      addUniqueTag(tags, seen, `Immune ${monster.damageImmunities}`)
    }

    if (monster.conditionImmunities) {
      addUniqueTag(tags, seen, `Condition Immune ${monster.conditionImmunities}`)
    }

    if (monster.senses) {
      addUniqueTag(tags, seen, `Senses ${monster.senses}`)
    }

    if (monster.languages) {
      addUniqueTag(tags, seen, `Languages ${monster.languages}`)
    }

    if (monster.habitat) {
      addUniqueTag(tags, seen, `Habitat ${monster.habitat}`)
    }
  }

  if (Array.isArray(existingTags)) {
    existingTags.forEach((entry) => {
      if (typeof entry === 'string') {
        addUniqueTag(tags, seen, entry)
      } else {
        const normalized = normalizeTagObject(entry)
        if (normalized) {
          addUniqueTag(tags, seen, normalized.label, {
            key: normalized.key,
...
export const parseDndBeyondMonster = (markdown, sourceUrl) => {
  ...
  let name = ''
  let nameIndex = -1
  let typeReferenceIndex = -1

  const titleIndex = relevantLines.findIndex((line) =>
    /^title:/i.test(line.trim()),
  )

  if (titleIndex !== -1) {
    const titleLine = relevantLines[titleIndex]
    const candidate = stripFormatting(titleLine.replace(/^Title:\s*/i, ''))
    if (candidate) {
      name = candidate
  ...
  const typeSearchStart =
    typeReferenceIndex !== -1 ? typeReferenceIndex : Math.max(0, nameIndex)

  let typeLine = ''
  for (let idx = typeSearchStart + 1; idx < relevantLines.length; idx += 1) {
    ...
    if (lower.startsWith('title:') || lower.startsWith('url source')) {
      continue
    }
    ...
    if (lower.startsWith('ac ') || lower.startsWith('armor class')) {
      break
    }

    if (isLikelyTypeLine(value)) {
      typeLine = value
      break
    }
  }
  ...
  if (!typeLine) {
    for (let idx = typeSearchStart + 1; idx < relevantLines.length; idx += 1) {
      const fallback = stripFormatting(relevantLines[idx])
      if (!fallback) {
        continue
      }
